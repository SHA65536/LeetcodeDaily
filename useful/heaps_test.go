...
func TestHeaps(t *testing.T) {
	...
	var dec = MakeHeap(MaxHeap)
	var inc = MakeHeap(MinHeap)
	var prev, cur int
	for i := range original {
		heap.Push(dec, original[i])
		heap.Push(inc, original[i])
	}
	prev = heap.Pop(dec).(int)
	for dec.Len() > 0 {
		cur = heap.Pop(dec).(int)
		if cur > prev {
			t.FailNow()
		}
	}
	prev = heap.Pop(inc).(int)
	for inc.Len() > 0 {
		cur = heap.Pop(inc).(int)
		if cur < prev {
			t.FailNow()
		}
	}
